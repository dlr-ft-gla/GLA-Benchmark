...
for jjj=1:length(load_types_to_be_analyzed)
    load_types.config(jjj).identifier = load_types_to_be_analyzed{jjj};
    load_types.config(jjj).directory  = [ResultsDirectory_Robust,filesep,load_types.config(jjj).identifier];
    load_types.config(jjj).file_list  = dir([load_types.config(jjj).directory,filesep,'*.mat']);
end

Compute_PerformanceStatistics;

Task_List_Discrete = Robust_Task_List_DG;
Task_List_Continuous = Robust_Task_List_CT;
save(['results',filesep,ResultsDirectoryName,filesep,'Robust_Performance_Statistics.mat'], 'Evaluations', 'Task_List_Discrete', 'Task_List_Continuous', 'load_types')
