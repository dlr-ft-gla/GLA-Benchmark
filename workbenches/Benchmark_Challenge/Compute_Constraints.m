...
constraint_max_rob_DG = [];
constraint_min_rob_DG = [];
constraint_max_rob_CT = [];
constraint_min_rob_CT = [];
constraint_limit_rob_CT = [];
constraint_peak_rob = [];
PIP_rob = [];
for ii = 1:length(Rob_Eval.Configs_List)
    if ~strcmp(Rob_Eval.Configs_List(ii).Identifier, 'Controller_Config__OL')
        max_tmp_DG = Rob_Eval.Configurations(ii).CS25_341a.Outputs.max_max(constraint_idxs);
        min_tmp_DG = Rob_Eval.Configurations(ii).CS25_341a.Outputs.min_min(constraint_idxs);
        max_tmp_CT = Rob_Eval.Configurations(ii).CS25_341b.Outputs.max_max(constraint_idxs);
        min_tmp_CT = Rob_Eval.Configurations(ii).CS25_341b.Outputs.min_min(constraint_idxs);
        limit_tmp_CT = U_sigma*(Rob_Eval.Configurations(ii).CS25_341b.Outputs.std_max(constraint_idxs)...
                    ./Rob_Eval.Configurations(ii).CS25_341b.Outputs.std_max(vgust_idx));
        
        PIP_rob = [PIP_rob; Rob_Eval.Configurations(ii).CS25_341b.Outputs.PIP_max];        

        constraint_peak_rob = [constraint_peak_rob; max(abs([[max_tmp_DG; min_tmp_DG].*envelope_DG_switch; limit_tmp_CT.*envelope_CT_switch]),[],1)];
        constraint_max_rob_DG = [constraint_max_rob_DG; max_tmp_DG];
        constraint_min_rob_DG = [constraint_min_rob_DG; min_tmp_DG];
        constraint_max_rob_CT = [constraint_max_rob_CT; max_tmp_CT];
        constraint_min_rob_CT = [constraint_min_rob_CT; min_tmp_CT];
        constraint_limit_rob_CT = [constraint_limit_rob_CT; limit_tmp_CT];
...
Constraint_Eval.Robust.Peak = constraint_peak_rob;
Constraint_Eval.Robust.DG.Min = constraint_min_rob_DG;
Constraint_Eval.Robust.DG.Max = constraint_max_rob_DG;
Constraint_Eval.Robust.CT.Min = constraint_min_rob_CT;
Constraint_Eval.Robust.CT.Max = constraint_max_rob_CT;
Constraint_Eval.Robust.CT.Limit = constraint_limit_rob_CT;

%% Print limit values
disp('Constrained variable limit values:');
disp('Limit  |  Open-Loop  |  Nominal  |  Robust');
disp('==========================================');
for jj = 1:length(Config_List_GLA)
    ...
    for ii = 1:length(constraint_names)
            ...
            disp([num2str(constraint_envelope(1, ii), '%10.2e\n'), ' | ', num2str(constraint_peak_OL(1, ii), '%10.2e\n'), ' | ',...
                num2str(constraint_peak_nom(jj, ii), '%10.2e\n'), ' | ', num2str(constraint_peak_rob(jj, ii), '%10.2e\n')]); 
            disp('-----------------------------------------');
    end
end
...
clear constraint_idxs constraint_envelope constraint_envelope_multiplier constraint_envelope_constant envelope_CT_switch envelope_DG_switch...
    constraint_max_nom_DG constraint_max_OL_DG constraint_max_rob_DG constraint_min_nom_DG constraint_min_OL_DG constraint_min_rob_DG...
    constraint_max_nom_CT constraint_max_OL_CT constraint_max_rob_CT constraint_min_nom_CT constraint_min_OL_CT constraint_min_rob_CT...
    constraint_limit_OL_CT constraint_limit_nom_CT constraint_limit_rob_CT...
    constraint_names constraint_peak_nom constraint_peak_OL constraint_peak_rob ...
    PIP_OL PIP_nom PIP_rob PIP_positions...
    U_sigma vgust_idx alt_tmp ii jj Nom_Eval Rob_Eval idx_OL Config_List_GLA  max_tmp min_tmp...
